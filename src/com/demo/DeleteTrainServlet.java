...
@WebServlet(name = "DeleteTrainServlet", urlPatterns = {"/content/DeleteTrainServlet"})
...
//            boolean result = userService.addTicketNumber(coach.getID(), coach.getGrade());
            request.setAttribute("dlt-message", "删除成功，欢迎下次选购！");
        } else if(flag == 0){
            request.setAttribute("dlt-message", "余票不足，请重新选购！");
        } else if(flag == 2){
            request.setAttribute("dlt-message", "您已购买过该列车车票！");
        }
        request.getRequestDispatcher("dTicket.jsp").forward(request, response);
    }
